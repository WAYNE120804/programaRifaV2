...
const getAsignacionesHistory = async (req, res) => {
    try {
        const { id } = req.params; // rifa_vendedor_id
        const history = await AsignacionBoletas.findAll({
            where: { rifa_vendedor_id: id },
            order: [['fecha', 'DESC']]
        });
        res.json(history);
    } catch (error) {
